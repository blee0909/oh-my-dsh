...
export async function syncTools(
  ...
  for (const tool of response.tools) {
    ...
    definitions.set(publicName, createMcpToolDefinition(ctx, {
      ...
      ...opts.imageStorageTimeoutMs !== undefined ? { imageStorageTimeoutMs: opts.imageStorageTimeoutMs } : {},
...
export interface McpToolDefinitionOptions {
  ...
  /** Timeout in milliseconds for persisting decoded MCP images to durable storage. Default: 30,000. */
  imageStorageTimeoutMs?: number | undefined
  /**
...
function createExecutor(
  ...
  return async (args: unknown, exec: ToolExecution) => {
    ...
    if (containsImage(content)) {
      ...
      const projected = await prepareImageProjection(ctx, exec, content, rawName, options.imageStorageTimeoutMs)
