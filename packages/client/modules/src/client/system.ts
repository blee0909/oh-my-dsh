...
export class ClientModuleSystem implements ClientModuleLoader {
  ...
  private async arrive(row: BootModuleRow): Promise<void> {
    ...
    try {
      await this.arriveUrl(url, id, reload)
    } catch (primaryError) {
      ...
      if (url === row.initialUrl && row.url !== row.initialUrl) {
        try {
          await this.arriveUrl(row.url, id, reload)
          return
        } catch {
      ...
      try {
        await this.arriveUrl(cacheBustUrl, id, reload)
        return
      } catch {
        throw primaryError
      }
  ...
  private arriveUrl(url: string, id: string, reload?: { url: string; rev: string }): Promise<void> {
