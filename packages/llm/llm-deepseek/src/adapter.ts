...
import { deepSeekFetch, providerError, providerErrorDetail } from './transport.ts'

function isAttachmentError(error: unknown): error is { code: string; message: string } {
  return (
    typeof error === 'object' &&
    error !== null &&
    'code' in error &&
  ...
  private async * request(
    ...
    try {
      ...
      while (true) {
        ...
        const response = await deepSeekFetch(`${messagesApiRoot(connection.baseURL)}/messages`, {
