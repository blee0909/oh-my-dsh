...
export class LlmRuntime extends TypertRemoteService {
  ...
  private async * adapterStream(
    ...
    try {
      ...
      const outboundMessages = transformMessages(projectedMessages as Message[], {
        allowInHistorySystem: adapterSystemPromptUpdate === 'in-history' || options.purpose === 'compaction',
      })
      let projectedOptions = resolvedOptions
      if (outboundMessages !== resolvedOptions.messages || projectedTools.tools !== resolvedOptions.tools) {
        projectedOptions = {
          ...resolvedOptions,
          messages: outboundMessages as RequestMessage[],
