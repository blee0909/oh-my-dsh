...
| `maxBatchBytes` | `4194304` | 每次请求贡献的会话日志事件序列化 JSON UTF-8 最大字节大小。超出上限的后缀会在后续请求中分批逐步消化。 |
